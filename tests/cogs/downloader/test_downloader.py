...
from Thinslave.cogs.downloader.repo_manager import RepoManager, Repo
from Thinslave.cogs.downloader.errors import ExistingGitRepo
...
async def test_add_repo(monkeypatch, repo_manager):
    monkeypatch.setattr("Thinslave.cogs.downloader.repo_manager.Repo._run", fake_run_noprint)
...
async def test_lib_install_requirements(monkeypatch, library_installable, repo, tmpdir):
    monkeypatch.setattr("Thinslave.cogs.downloader.repo_manager.Repo._run", fake_run_noprint)
    monkeypatch.setattr(
        "Thinslave.cogs.downloader.repo_manager.Repo.available_libraries", (library_installable,)
    )
