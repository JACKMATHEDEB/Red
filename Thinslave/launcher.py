...
from Thinslave import MIN_PYTHON_VERSION
from Thinslave.setup import (
    basic_setup,
...
from Thinslave.core import __version__, version_info as red_version_info, VersionInfo
from Thinslave.core.cli import confirm
...
def parse_cli_args():
    parser = argparse.ArgumentParser(
        description="Thinslave - Discord Bot's launcher (V3)", allow_abbrev=False
    )
    ...
    parser.add_argument("--start", "-s", help="Starts Thinslave", action="store_true")
    parser.add_argument(
        "--auto-restart", help="Autorestarts Red in case of issues", action="store_true"
    )
    parser.add_argument("--update", help="Updates Thinslave", action="store_true")
    parser.add_argument(
        "--update-dev", help="Updates Thinslave", action="store_true"
    )
...
def update_red(dev=False, style=False, mongo=False, docs=False, test=False):
    ...
    print("Updating Red...")
    # If the user ran Thinslave-launcher.exe, updating with pip will fail
    # on windows since the file is open and pip will try to overwrite it.
    # We have to rename Thinslave-launcher.exe in this case.
    launcher_script = os.path.abspath(sys.argv[0])
    old_name = launcher_script + ".exe"
    new_name = launcher_script + ".old"
    renamed = False
    if "Thinslave-launcher" in launcher_script and IS_WINDOWS:
        renamed = True
    ...
    if test:
        egg_l.append("test")
    if dev:
        package = "git+https://github.com/Thinmatrix/Thinslave-DiscordBot@V3/develop"
        if egg_l:
            package += "#egg=Thinslave-DiscordBot[{}]".format(", ".join(egg_l))
    else:
        package = "Thinslave-DiscordBot"
        if egg_l:
    ...
    # If Thinslave wasn't updated, we renamed our .exe file and didn't replace it
    scripts = os.listdir(os.path.dirname(launcher_script))
    if renamed and "Thinslave-launcher.exe" not in scripts:
...
def run_red(selected_instance, autorestart: bool = False, cliflags=None):
    ...
    while True:
        ...
        cmd_list = [interpreter, "-m", "Thinslave", selected_instance]
        if cliflags:
            cmd_list += cliflags
...
def cli_flag_getter():
    print("Would you like to enter any cli flags to pass to Thinslave? (y/n)")
    resp = user_choice()
    if resp == "n":
        return None
    elif resp == "y":
...
def debug_info():
    pyver = sys.version
    redver = pkg_resources.get_distribution("Thinslave-DiscordBot").version
    if IS_WINDOWS:
